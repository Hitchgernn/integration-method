...
{\large \textbf{INTEGRASI NUMERIS DENGAN METODE EXTRAPOLATION, ROMBERG INTEGRATION, ADAPTIVE INTEGRATION, DAN GAUSSIAN QUADRATURE}}\\[2cm] % 
...
Kode berikut mengimplementasikan Aturan Trapesium Komposit untuk kedua fungsi. %  

\begin{lstlisting}[language=Python, caption={Implementasi metode Trapezoidal}, label=list:trap]
import math
PI = math.pi
...
Kode berikut mengimplementasikan Ekstrapolasi Richardson, yang menggunakan fungsi-fungsi dari. Parameter $p$ adalah orde dari error (untuk Trapesium, $p=2$) dan $n$ adalah jumlah interval awal. % 

\begin{lstlisting}[language=Python, caption={Implementasi metode Ekstrapolasi Richardson}, label=list:extrap]
...
\subsection{Implementasi Kode} % 
Kode berikut mengimplementasikan fungsi Romberg secara rekursif dan membangun tabel $3 \times 3$ (hingga $R(2,2)$). % 
\begin{lstlisting}[language=Python, caption={Implementasi metode Integrasi Romberg}, label=list:romberg] 
from trapezoidal import *
...
\subsection{Implementasi Kode} % 
Kode berikut mengimplementasikan metode adaptif berbasis Simpson. % 
\begin{lstlisting}[language=Python, caption={Implementasi metode Adaptif Integral}, label=list:adaptive]
import math
PI = math.pi
...
\subsection{Implementasi Kode} % 
Berikut adalah kode implementasi Kuadratur Gauss menggunakan data $x_i$ dan $w_i$ dari tabel \ref{tab:parameter_gauss} sebagai parameter. 
...
\begin{figure}[H]
    ...
    % --- Kode Pertama ---
    \begin{lstlisting}[language=Python, caption={IImplementasi metode Kuadratur Gauss}, label=list:gaussnolib] 
import math
PI = math.pi
...
\begin{figure}[H]
    ...
    % --- Kode Pertama ---
    \begin{lstlisting}[language=Python, caption={Implementasi metode Kuadratur Gauss dengan \textit{library numpy}}, label=list:gausswithlib] 
import numpy as np
import math
pi = math.pi
...
\subsection{Hasil Perhitungan dan Analisis Error} % 
Hasil dari kode yang menggunakan \textit{library numpy} lebih akurat. Hal ini mungkin terjadi karena nilai koefisien yang didapat dari algoritma di dalamnya lebih akurat daripada koefisien yang di atur manual. Oleh karena itu, analisis yang dilakukan adalah hasil output dari kode yang menggunakan \textit{library numpy}. Berikut adalah output dari kode di tersebut dari $n=1$ sampai $n=5$ titik: % 
