...
% --- Judul Utama ---
{\Large \textbf{PROJEK METODE NUMERIS}}\\[1cm] % 
...
Kode berikut (dari \texttt{Integration.py}) mengimplementasikan Aturan Trapesium Komposit untuk kedua fungsi. % 
...
# Output  
yang dihasilkan oleh loop print  
...
\subsection{Pengertian dan Rumus} % 
Kuadratur Gauss (secara spesifik Gauss-Legendre) adalah metode yang sangat berbeda dan kuat. % 
Alih-alih menggunakan titik-titik yang berjarak sama (seperti Trapesium/Simpson), metode ini secara cerdas memilih $n$ titik (disebut \textbf{nodes}, $x_i$) dan \textbf{bobot} ($w_i$) yang optimal. % 
\begin{itemize}
...
\begin{lstlisting}[language=Python, caption={Kode gauss_1.py}, label=list:gauss1] 
import numpy as np
import math
pi = math.pi
...
\end{lstlisting}
\begin{lstlisting}[language=Python, caption={Kode gauss_2.py}, label=list:gauss2]
import numpy as np
import math
pi = math.pi
